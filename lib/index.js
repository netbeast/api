...
function netbeast (top) {
  ...
  var core = {
    ...
    discoverDevices: function (app) {
      var apps = []
      var promise = new Promise(function (resolve, reject) {
        NETBEAST='localhost:8000'
        request.get('http://' + NETBEAST + '/api/plugins')
        .then(function (res) {
    ...
    get: function (args) {
      ...
      return request.get('http://localhost:8000/api/resources')
      .then(function (res) {
        ...
        return Promise.map(res.body, function (item, done) {
          return request.get('http://localhost:8000/i/' + item.app + item.hook).query(queryString)
        .then(function (res) {
    ...
    getById: function (id) {
      return request.get('http://localhost:8000/api/resources').query({ id: id })
      .then(function (res) {
        if (!res.body.length) return Promise.reject(new Error('These resources doesn´t exists!'))
        var item = res.body[0]
        return request.get('http://localhost:8000/i' + item.app + item.hook)
        .then(function (res) {
    ...
    set: function (args) {
      ...
      .then(function (res) {
        ...
        return Promise.map(res.body, function (item, done) {
          return request.post('http://localhost:8000/i/' + item.app + item.hook).send(args)
          .then(function (res) {
            item.result = (Object.keys(res.body).length) ? res.body : res.text
            return Promise.resolve(item)
          })
