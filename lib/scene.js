            ...
            request.get({
              ...
              if (err) reject(err)
                ...
                json: {
                  ...
                  brightness: body.data.brightness,
                  hue: body.data.hue,
                  saturation: body.data.saturation
                }},
                function (err, resp, body) {
                  if (err) reject(err)
      ...
      var promise = new Promise(function (resolve, reject) {
        ...
        .then(function (devices) {
          devices.data.forEach(function (device) {
            resource().setById(device.id, {power: device.power, brightness: device.brightness, hue: device.hue, saturation: device.saturation})
            .then(function (data) {
              resolve({error: {}, data: true})
            }).catch(function (data) {
              reject(data)
